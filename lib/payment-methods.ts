...
/**
 * The three payment rails the checkout can present. Keep this in sync
 * with `app/actions/place-order.ts#ALLOWED_PAYMENT_METHODS`.
 */
export type PaymentMethodKey = "card" | "zelle" | "crypto"
...
/**
 * Default to "everything on" so a missing settings row (fresh install,
 * failed read) never locks customers out of checkout. Admins have to
 * deliberately opt a method out.
 */
